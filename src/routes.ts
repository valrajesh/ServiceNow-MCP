...
export function buildRouter(manager: MCPManager, getCreds: () => SNCredentials): Router {
  ...
  function endpointUrl(req: Request, serverName: string): string {
  ...
  r.get("/servers", (_req, res) => {
    ...
    res.json(
      servers.map((s) => ({
        ...
        endpointUrl: endpointUrl(_req, s.name),
      }))
    );
  });
  ...
  r.get("/servers/:id", (req, res) => {
    ...
    res.json({ ...server, running: manager.isRunning(server.id), endpointUrl: endpointUrl(req, server.name) });
  });
  ...
  r.post("/servers", async (req, res) => {
    ...
    res.status(201).json({ ...server, running: true, endpointUrl: endpointUrl(req, server.name) });
  });
  ...
  r.get("/servers/:id/config-snippet", (req, res) => {
    const server = storage.getServer(req.params["id"]!);
    if (!server) { res.status(404).json({ error: "Not found" }); return; }
    const url = endpointUrl(req, server.name);
